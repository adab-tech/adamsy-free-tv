...
Source: "{#StagingDir}\VirtualTVClassic.exe"; DestDir: "{app}\dist"; Flags: ignoreversion
...
Source: "{#StagingDir}\start_tv_app_classic.bat"; DestDir: "{app}"; Flags: ignoreversion
...
Name: "{autoprograms}\{#AppName} (Classic)"; Filename: "{app}\start_tv_app_classic.bat"; WorkingDir: "{app}"; IconFilename: "{app}\dist\VirtualTVClassic.exe"; Comment: "Launch {#AppName} Classic (Tkinter/VLC)"
...
begin
  ...
  begin
    MsgBox(
      'VLC media player was not detected. The default Adamsy Free TV app does not need it, ' +
      'but the "Adamsy Free TV (Classic)" shortcut requires VLC - install it before using Classic mode.',
      mbInformation,
      MB_OK
    );
  end;
